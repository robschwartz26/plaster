...
function PanelCard({ header, children, bodyPadding = 16 }: {
...
// ── Layout persistence (separate keys per role) ──────────────
...
export function StaffScreen() {
  ...
  // ── Top bar ──────────────────────────────────────────────
  const topBar = (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 24px', paddingTop: 'calc(env(safe-area-inset-top) + 12px)', borderBottom: '1px solid var(--fg-08)', background: 'var(--bg)', flexShrink: 0 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <span style={{ fontFamily: '"Playfair Display", serif', fontSize: 26, fontWeight: 900, color: 'var(--fg)', letterSpacing: '-0.02em', lineHeight: 1 }}>plaster</span>
  ...
  // ── defaultSizes: worker 3-panel vs admin 4-panel ────────
  // Worker:  ingester=30  board=48           team=22
  // Admin:   ingester=24  board=34  review=24 team=18
  const sizes = isAdmin
    ? { ingester: 24, board: 34, review: 24, team: 18 }
    : { ingester: 30, board: 48, team: 22 }
      ...
      {isWide ? (
        ...
        <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'row', padding: 12, gap: 0, overflow: 'hidden' }}>
          ...
          <div style={{ width: 360, flexShrink: 0, minHeight: 0, marginRight: 8, display: 'flex', flexDirection: 'column', border: '1px solid var(--fg-15)', borderRadius: 12, background: 'var(--bg)', overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, minHeight: 0, overflow: 'hidden' }}><StaffPreview /></div>
          </div>

          {/* Resizable group */}
          <div style={{ flex: 1, minWidth: 0, minHeight: 0 }}>
            <PanelGroup orientation="horizontal" defaultLayout={savedLayout} onLayoutChanged={saveLayout}>

              <ResizablePanel id="ingester" defaultSize={sizes.ingester} minSize={15} style={{ overflow: 'hidden' }}>
                <PanelCard header="Add a show"><ImportForm staffMode /></PanelCard>
              </ResizablePanel>

              <ResizeSeam />

              <ResizablePanel id="venue-board" defaultSize={sizes.board} minSize={22} style={{ overflow: 'hidden' }}>
                <PanelCard bodyPadding={16}><VenueBoard /></PanelCard>
              </ResizablePanel>

              {isAdmin && (
                <>
                  <ResizeSeam />
                  <ResizablePanel id="review" defaultSize={(sizes as { review: number }).review} minSize={18} style={{ overflow: 'hidden' }}>
                    <PanelCard header="Review"><AdminPendingEvents /></PanelCard>
                  </ResizablePanel>
                </>
              )}
              ...
              <ResizablePanel id="team" defaultSize={sizes.team} minSize={12} style={{ overflow: 'hidden' }}>
                <PanelCard header="Team">{teamContent}</PanelCard>
              </ResizablePanel>
        ...
        <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', gap: 12, padding: 12, overflowY: 'auto' }}>
          ...
          {isAdmin && (
            <PanelShell header="Review"><AdminPendingEvents /></PanelShell>
          )}
