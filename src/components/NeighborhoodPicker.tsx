...
import { SEXTANTS, SEXTANT_LABELS, SUBURBS, neighborhoodsBySextant, type Sextant } from '@/lib/neighborhoods'

const SUBURBS_SORTED = [...SUBURBS].sort((a, b) => a.name.localeCompare(b.name))
...
export function NeighborhoodPicker({ value, onChange }: {
  ...
  const suburbItems = SUBURBS_SORTED.filter(n => !q || n.name.toLowerCase().includes(q))
  const anyMatch = suburbItems.length > 0
    || SEXTANTS.some(sx => neighborhoodsBySextant(sx).some(n => !q || n.name.toLowerCase().includes(q)))
  ...
  return (
    <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: 10 }}>
      <input
        ...
        placeholder="Search neighborhoods… (e.g. Kenton or Beaverton)"
      ...
      <div style={{ maxHeight: 300, overflowY: 'auto', border: '1px solid var(--fg-15)', borderRadius: 10 }}>
        ...
        {/* Suburbs & surrounding towns — chip shows the town, wall scopes to
            the nearest Portland sextant */}
