...
export function AdminCommunityPosts() {
  ...
  const fetchPending = useCallback(async () => {
    ...
    setRows((data ?? []) as unknown as CommunityPost[])
    setLoading(false)
  }, [])
    ...
    <div style={{ marginBottom: 20 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {rows.map(p => {
          ...
          return (
            <div key={p.id} style={{ display: 'flex', gap: 10, padding: 10, borderRadius: 10, border: `1px solid ${p.flagged ? 'rgba(239,68,68,0.35)' : 'var(--fg-15)'}`, background: p.flagged ? 'rgba(239,68,68,0.04)' : 'transparent' }}>
