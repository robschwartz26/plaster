...
export function FlyerCarousel({ event, isLiked, onLike, onClose, onVenueTap }: Props) {
  ...
  // ── Carousel strip (4 panels: Flyer, Info, PostWall, FlyerClone) ──────────
  ...
  // ── Extra event detail (fetched on mount) ──────────────────────────────
  ...
  // ── On mount: increment view count + fetch data ────────────────────────
  useEffect(() => {
    // Increment view count (silently ignore if RPC doesn't exist yet)
    supabase.rpc('add_view_count', { p_event_id: event.id, delta: 1 }).then(() => {
      setLocalViewCount((v) => v + 1)
    })

    // Extra event details (description, address, ends_at)
    supabase.from('events')
      .select('description, address, ends_at')
      .eq('id', event.id)
      .single()
  ...
  function snapTo(idx: number, animate = true) {
    ...
    el.style.transition = animate
      ...
      : 'none'
    // Each panel is 25% of the 400%-wide strip → shifting by -25% per panel
    el.style.transform = `translateX(${-idx * 25}%)`
    setStripIdx(idx)
    // Seamless loop: after snapping to clone (panel 3), instantly reset to panel 0
    if (idx === 3) {
      setTimeout(() => {
  ...
  // ── Touch handlers (registered once on overlay) ────────────────────────
  useEffect(() => {
    ...
    const onStart = (e: TouchEvent) => {
      if (e.touches.length === 2) {
        // Pinch zoom — only on flyer panels (index 0 or 3)
        const idx = stripIdxRef.current
        if (idx !== 0 && idx !== 3) return
        const t0 = e.touches[0], t1 = e.touches[1]
        const dist = Math.hypot(t0.clientX - t1.clientX, t0.clientY - t1.clientY)
        // Find the visible flyer image
        const panels = stripRef.current?.children
        const panelEl = panels?.[idx] as HTMLElement | undefined
    ...
    const onMove = (e: TouchEvent) => {
      ...
      // Determine horizontal vs vertical on first significant movement
      ...
      if (swipeRef.current.isHorizontal === false) {
        // Vertical — let the panel scroll handle it
        swipeRef.current.active = false
        return
      }
      ...
      if (swipeRef.current.isHorizontal === true) {
        ...
        const el = stripRef.current
        if (!el) return
        // Base position in px, drag offset added
    ...
    const onEnd = (e: TouchEvent) => {
      ...
      if (!swipeRef.current.active || swipeRef.current.isHorizontal !== true) return
      ...
      let next = cur
      if (dx < -50) next = Math.min(3, cur + 1)   // swipe left → advance
      else if (dx > 50) next = Math.max(0, cur - 1) // swipe right → back
  ...
  function handleLike() {
    if (!user) return
    onLike(event.id) // update Wall's state
    if (localIsLiked) {
  ...
  // ── Shared close button ────────────────────────────────────────────────
  const CloseBtn = ({ style }: { style?: React.CSSProperties }) => (
    <button
      onClick={onClose}
      style={{
        position: 'absolute',
  ...
  // ── Panel 1: Flyer ─────────────────────────────────────────────────────
  const FlyerPanel = ({ isClone = false }: { isClone?: boolean }) => (
    <div
      style={{
        width: '25%',
        height: '100%',
      ...
      onClick={!isClone ? handleFlyerTap : handleFlyerTap}
    >
      {event.poster_url ? (
        <img
      ...
      >
        ...
        {/* Like */}
        <button style={{ ...pillStyle, cursor: 'pointer', border: 'none' }} onClick={(e) => { e.stopPropagation(); handleLike() }}>
          <svg
            width="12"
            height="11"
            viewBox="0 0 24 22"
  ...
  const InfoPanel = () => (
    ...
    >
      ...
      <div style={{
        ...
        <CloseBtn style={{ position: 'absolute', top: 'max(14px, env(safe-area-inset-top))', left: 14, background: 'var(--fg-08)' }} />
        <Dots />
      </div>
